...
function hamburgerClick(){
  ...
  if (clicked == false){
    ...
    TweenMax.to(hamSVG.querySelectorAll(".st1"), 0.5, {stroke: "#FFF", ease:Power1.easeOut});
    ...
    TweenMax.to(hamSVG.querySelectorAll(".st1"), 0.5, {stroke: hamburgerColor, ease:Power1.easeOut});
...
function adjustSquare(){
  ...
  document.querySelector('#aboutText').style.height = "500px";
...
function checkScroll(){
  ...
  if (window.scrollY >= ((document.querySelector("#aboutHeader").offsetTop)-200)){
    //console.log("running");
    TweenMax.to(document.querySelector("#aboutHeader"), 1, {backgroundColor:"#0F7173", delay: 0.25, ease: Power1.easeOut});
    TweenMax.to(document.querySelector("#aboutHeaderContent"), 1, {opacity:1, delay: 0.5, ease: Power1.easeOut});
    if (!(window.matchMedia("(min-width: 48em)").matches)) {
...
if (!(window.matchMedia("(min-width: 48em)").matches)) {
    if(window.scrollY >= ((document.querySelector('#buttonSection').offsetTop)-200)){
  ...
  if(window.scrollY >= ((document.querySelector('#buttonSection').offsetTop)-400)){
    var boxes = document.querySelectorAll('.buttonContainer');
    TweenMax.to(document.querySelector('#buttonSection'), 1, {opacity:1, ease:Power1.easeOut});
    for (var i = 0; i < boxes.length; i++) {
      TweenMax.to(boxes[i], 0.5, {opacity:1, delay: (0.5+(i/2)), ease: Power1.easeOut});
    ...
    if (window.scrollY >= ((portfolio.offsetTop)-400)) {
      ...
      for (var i = 0; i < squareDivs.length; i++) {
        ...
        if (window.scrollY >= (squareDivs[i].offsetTop)) {
          TweenMax.to(squareDivs[i], 1, {opacity: 1, ease:Power1.easeOut, delay:(i/4)+0.5});
          TweenMax.to(portDivs[i], 1, {opacity: 1, ease:Power1.easeOut, delay: ((i/4)+1)});
          //console.log("ran");
        }
      }
    }
...
function hamburgerScroll(){
  ...
  //console.log("running");
    ...
    if (!(window.matchMedia("(min-width: 48em)").matches)) {
      if(window.scrollY < (document.querySelector('#aboutText').offsetTop)){
        //console.log("ran");
        TweenMax.to(hamSVG.querySelectorAll('.st1'), 0.5, {stroke:"#FFF", ease:Power1.easeOut});
        hamburgerColor = "#FFF";
      }
      else if(window.scrollY >= (document.querySelector('#aboutText').offsetTop)) {
        //console.log("changed colour");
        TweenMax.to(hamSVG.querySelectorAll('.st1'), 0.5, {stroke:"#0F7173", ease:Power1.easeOut});
      ...
      if(window.scrollY < ((document.querySelector('#aboutText').offsetTop)-30)){
        //console.log("ran");
        TweenMax.to(hamSVG.querySelectorAll('.st1'), 0.5, {stroke:"#FFF", ease:Power1.easeOut});
        hamburgerColor = "#FFF";
      }
      else if(window.scrollY >= ((document.querySelector('#aboutText').offsetTop)-30)) {
        //console.log("changed colour");
        TweenMax.to(hamSVG.querySelectorAll('.st1'), 0.5, {stroke:"#0F7173", ease:Power1.easeOut});
        hamburgerColor = "#0F7173";
...
function scrollAbout(e){
  ...
  TweenMax.to(window, 1, {scrollTo:about});
}
...
function scrollPortfolio(e){
  ...
  TweenMax.to(window, 1, {scrollTo:portfolio});
}
